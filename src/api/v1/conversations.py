...
from src.schemas.chat import ConversationDetail, ConversationList, ConversationMessage, ConversationSummary
...
async def get_conversations(
    patient_id: str, db: AsyncSession = Depends(get_db), current_user: UserInfo = Depends(get_current_user)
):
    result = await db.execute(
        select(Conversation).filter(Conversation.patient_id == patient_id).order_by(Conversation.last_message_at.desc())
    )
...
async def get_conversation_detail(
    ...
    if not conversation:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Không tìm thấy phiên hội thoại {conversation_id}"
        )
    ...
    return ConversationDetail(conversation_id=conversation.id, messages=messages_list)
