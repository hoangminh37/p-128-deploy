...
async def get_queue(status: EditorItemStatus = Query("pending"), db: AsyncSession = Depends(get_db)):
...
async def get_queue_item(item_id: str, db: AsyncSession = Depends(get_db)):
...
async def approve_queue_item(item_id: str, payload: EditorApproveRequest, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(EditorQueueItemModel).filter(EditorQueueItemModel.id == item_id))
    item = result.scalars().first()
    ...
    item.reviewed_by = "u_01HQZV" # Mock editor ID
    
    await db.commit()
    await db.refresh(item)
    
    return await get_queue_item(item_id, db)

@router.post("/queue/{item_id}/reject", response_model=EditorQueueItemDetail)
async def reject_queue_item(item_id: str, payload: EditorRejectRequest, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(EditorQueueItemModel).filter(EditorQueueItemModel.id == item_id))
    item = result.scalars().first()
...
async def get_out_of_scope_logs(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
...
async def draft_out_of_scope(log_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if log.drafted and log.drafted_item_id:
        ...
        return await get_queue_item(log.drafted_item_id, db)
    ...
    db.add(draft)
    await db.flush() # To get the draft.id
    
    log.drafted = True
    log.drafted_item_id = draft.id
    await db.commit()
    
    return await get_queue_item(draft.id, db)
