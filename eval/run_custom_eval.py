...
def run_custom_evaluation(mock: bool = False):
    ...
    if "answer" not in dataset_dict or len(dataset_dict["answer"]) == 0:
        print("❌ Dataset chưa có cột 'answer'. Vui lòng chạy eval/run_ragas_eval.py trước để Agent sinh ra câu trả lời.")
        if mock:
    ...
    class EvaluationResult(BaseModel):
        has_citation: int = Field(description="1 nếu câu trả lời CÓ trích dẫn nguồn (footnote/link/tên tài liệu). 0 nếu không có.")
        has_next_questions: int = Field(description="1 nếu câu trả lời CÓ gợi ý 3 câu hỏi tái khám/câu hỏi liên quan ở cuối. 0 nếu không có.")
        has_disclaimer: int = Field(description="1 nếu câu trả lời CÓ chứa câu miễn trừ trách nhiệm y tế (VD: Thông tin mang tính giáo dục, không thay thế tư vấn...). 0 nếu không.")
        tone_score: int = Field(description="Điểm từ 1 đến 5 đánh giá giọng điệu thấu cảm, dễ hiểu, lịch sự và không hù dọa bệnh nhân (5 là xuất sắc).")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mock", action="store_true", help="Chạy chế độ giả lập đánh giá 3 câu đầu để tiết kiệm token.")
