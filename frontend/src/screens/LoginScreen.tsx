        ...
        {DEMO_ACCOUNTS.map((account) => (
          <li key={account.email}>
            <button
              ...
              className="font-display flex min-h-touch w-full flex-col justify-center rounded-lg border-2 border-border px-snug py-tight text-left text-ink"
            >
...
export function LoginScreen() {
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-paper px-cozy py-block text-ink">
      <main className="w-full max-w-answer">
        <div className="flex items-center gap-tight">
          <AppMark className="h-8 w-8 shrink-0 text-medical" />
          <p className="font-display text-app font-bold">{APP_NAME}</p>
        </div>

        <h1 className="font-display mt-block text-ask font-bold">Đăng nhập</h1>
        ...
        <form onSubmit={onSubmit} noValidate className="mt-block">
          ...
          <button
            ...
            className="font-display mt-block min-h-touch w-full rounded-lg border-2 border-medical bg-medical px-cozy text-input font-bold text-paper disabled:border-rule disabled:bg-transparent disabled:font-normal disabled:text-moss"
          >
